...
model.var_popu_served = Var(model.hubs, initialize = 0.0, domain = NonNegativeReals)
...
hub_capacity_dict = {hub : 5*hub_occ_dict[hub] for hub in model.hubs}
...
model.con_min_coverage = Constraint(expr = sum(cengeo_pop_dict[cg]*model.var_prop_served[(cg, hub)] for cg, hub in model.cg_hub_nearby_pairs) >= 0.50*sum(cengeo_pop_dict[cg] for cg, hub in model.cg_hub_nearby_pairs))
...
model.con_max_hubs = Constraint(expr = sum(model.var_hub_yn[hub] for hub in model.hubs) <= 9)
...
for hub in model.hubs:
    ...
    for cg in model.cengeos:
        if (cg, hub) in model.cg_hub_nearby_pairs:
            ...
            tot_pop_served = tot_pop_served
    model.var_popu_served[hub] = tot_pop_served

def serve_less_than_occ(model, hubs):
    return model.var_pop_served[hub] <= model.param_max_occ[hub]
