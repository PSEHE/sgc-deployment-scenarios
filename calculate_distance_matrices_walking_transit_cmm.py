# This script uses the transit graph created in create_transit_graph_cmm.py
# to make a distance matrix (in seconds)
...
#G_proj = ox.project_graph(G, nad83)

#list(G.edges(data=True))[-1]
#list(G.edges(data=True))[0]
#G.graph["crs"]
#county_graph = ox.load_graphml(os.path.join(os.getcwd(), 'data', 'graphs', 'graph_walk_cf_transit_' + output_county + '.graphml'))

for county, county_fips in ca_counties.items():
    output_county = county.lower().replace(' ', '')
    output_file_name = os.path.join(os.getcwd(), 'data', 'distance_matrices', 'distmatrix_walk_cf_transit_' + output_county + '.csv')
    if not(os.path.exists(output_file_name)): # If distance matrix has not been calculated yet
        ...
        if not(os.path.exists(os.path.join(os.getcwd(), 'data', 'graphs', 'graph_walk_cf_transit_' + output_county + '.graphml'))): # If graph has not been downloaded yet
            print("Calculate graph for " + county + " first.")
        ...
        for _,bg_row in bgs_county_gdf.iterrows():
            ...
            sites_nearby = get_nearby_sites_lat_long(bg_row, sites_county_gdf, k_neighbors, miles_distance)
            ...
            for site in sites_nearby: # For each site
                ...
                try:
                    ...
                    travel_dist_sec = nx.shortest_path_length(county_graph, node_origin, node_target, weight = 'length')
