...
def add_demand_maximization_objective(model):
	##### DEFINE OBJECTIVE
	model.max_coverage = Objective(expr = sum(model.var_prop_bg_at_site[pair]*model.param_bg_pop[pair[0]] for pair in model.idx_bg_site_pairs), sense = maximize)
	return model
...
def get_vars(model):
    var_hub_yn = [model.var_hub_yn[site].value for site in model.idx_sites]
    var_hub_yn=pd.DataFrame(var_hub_yn,index=model.idx_sites)
    ...
    var_prop_served = pd.DataFrame(prop_served_list,index = model.idx_bgs)
    return var_hub_yn, var_prop_served
